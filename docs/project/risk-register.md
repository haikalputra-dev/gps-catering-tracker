...
| R-29 | Dispatch and completion require the courier to remember to tap the button in the field; a late tap yields a late `dispatched_at` or `delivered_at` timestamp. | Low | Accepted for prototype (ADR-015, AR-35). Auto-detection via GPS proximity is a future packet. Field workflow training addresses the operational side. |
| R-30 | Reassignment is not supported. A courier who becomes unavailable mid-route forces a cancel-and-re-draft cycle. | Low | Deliberate design (ADR-014, AR-36). Cancellation from `in_transit` (AR-38 revised) closes the loop; a fresh draft rebuilds the delivery for a different courier. |
| R-31 | Per-courier concurrency cap defaults to `1` and is enforced at scheduling only. Raising it via `DELIVERY_MAX_CONCURRENT_PER_COURIER` allows a courier to hold multiple active deliveries but does not add UI to hand them off or reprioritise them. | Low | Cap is env-configurable per AR-34; the dashboard renders whichever active deliveries the query returns. Multi-drop UX is out of scope for Packet 09. |
| R-32 | Fee privacy on the courier show page is enforced by a Blade conditional. A future refactor that inlines the show template or bypasses the conditional could leak the fee. | Medium | Locked by `DeliveryFeePrivacyForCourierTest` (assigned courier sees no `Pricing`, no `Distance:`, no formatted fee; owner and staff still do). Any template change must keep the tests green. |
