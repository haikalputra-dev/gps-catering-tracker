/**
 * Customer coordinate-selection map.
 *
 * Reads its configuration from a `#customer-map` element's data
 * attributes and updates the hidden coordinate inputs
 * (`#customer-latitude` and `#customer-longitude`) as the user clicks
 * the map or drags the marker.
 *
 * This module assists input only; Laravel is the coordinate authority.
 * It intentionally does not share state with kitchen-map.js.
 */

import L from 'leaflet';
import 'leaflet/dist/leaflet.css';

// Ensure Leaflet's default marker icons resolve through Vite. Without
// this, the marker image URLs are undefined because Leaflet's stock
// code uses a runtime path calculation that breaks under a bundler.
...
function readConfig(container) {
    const cfg = {
        ...
        tileAttribution: container.dataset.tileAttribution
...
function initCustomerMap() {
    ...
    const displayEl = document.getElementById('customer-coordinate-display');
            ...
            if (lat === null || lng === null) {
                ...
                if (displayEl) {
                    displayEl.textContent
                        = 'Please select a coordinate on the map before saving.';
                }
            }
        });
    }
